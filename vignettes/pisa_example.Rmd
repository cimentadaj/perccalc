...
pisa_2012 <-
  ...
  mutate(father_edu = factor(father_edu, levels = order_edu, ordered = TRUE),
         avg_math = rowMeans(pisa_2012[c("PV1MATH", "PV2MATH", "PV3MATH", "PV4MATH", "PV5MATH")]))

pisa_2006 <-
  pisa_2006 %>%
  mutate(father_edu = factor(father_edu, levels = order_edu, ordered = TRUE),
         avg_math = rowMeans(pisa_2006[c("PV1MATH", "PV2MATH", "PV3MATH", "PV4MATH", "PV5MATH")]))
...
cnt_diff <-
  ...
  mutate(edu_diff = lapply(data, function(x) df_wrap(perc_diff(x, father_edu, avg_math, percentiles = c(90, 10))))) %>%
  select(-data) %>% 
  unnest(edu_diff)
...
cnt_half <-
  ...
  mutate(edu_diff = lapply(data,
                           function(x) df_wrap(perc_diff(x, father_edu, avg_math, percentiles = c(90, 50))))) %>%
  select(-data) %>% 
  unnest(edu_diff)
...
cnt_bottom <-
  ...
  nest(data = c(-year, -CNT)) %>%
  mutate(edu_diff = lapply(data,
                           function(x) df_wrap(perc_diff(x, father_edu, avg_math, percentiles = c(50, 10))))) %>%
  select(-data) %>% 
  unnest(edu_diff)
...
cnt_dist <-
  pisa %>%
  nest(data = c(-year, -CNT)) %>%
  mutate(edu_diff = lapply(data, function(x) df_wrap(perc_dist(x, father_edu, avg_math)))) %>%
  select(-data) %>% 
  unnest(edu_diff)
