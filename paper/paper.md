...
* Calculating values for a continuous variable relative to the percentiles of an
  ordered categorical variable (values of a continuous variable for the 1th, 10th,
...
points. We can extrapolate this example for each country separately using `perc_diff_df`
...
In contrast, `perc_dist` calculates the distribution of the percentiles which
is useful for comparing more finegrained percentiles rather than differences.
